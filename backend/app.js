...
const {
  router: artigoRoutes,
  setSocketIO: setArtigoSocketIO
} = require('./routes/artigoRoutes');
...
const deviceActivationRoutes =
  require('./routes/deviceActivationRoutes');

const rfidRoutes =
  require('./routes/rfidRoutes');

const configurarSocketRevisaoRFID =
  require('./socket/revisaoRFIDSocket');

const deviceProvisioningRoutes =
  require('./routes/deviceProvisioningRoutes');

const configurarSocketProvisioning =
  require('./socket/deviceProvisioningSocket');

const ProducaoDetalhada =
  require('./models/ProducaoDetalhada');
...
configurarSocketProvisioning(io);
...
io.on('connection', (socket) => {
  ...
  const buildDeviceStatusPayload =
    async (dispositivo) => {
        ...
        socket.emit(
          'artigoSelecionado',
          {
            data: {
              ...
              artigo: null,
              error:
                'Erro interno ao selecionar artigo'
            }
          }
        );
      }
      ...
      try {
        ...
        if (dispositivo) {
          ...
          socket.emit(
            'deviceRegistered',
            {
              success: true,
              message:
                'Dispositivo registrado e vinculado!',
              data: {
                deviceToken,
                usuarioVinculado:
                  true
          ...
          socket.emit(
            'deviceRegistered',
            {
              success: true,
              message:
                'Dispositivo conectado, aguardando vínculo.',
              data: {
                deviceToken,
                usuarioVinculado:
                  false
        ...
        socket.emit(
          'deviceRegistered',
          {
            success: false,
            message:
              'Erro interno ao registrar dispositivo.',
            data: {
              deviceToken:
                data?.deviceToken ||
                null,
      ...
      try {
        ...
        if (!dispositivo.usuario) {
          socket.emit(
            ...
            {
              message:
                'Dispositivo não vinculado a um usuário.'
            }
          );
          return;
        }
  ...
  socket.on(
    ...
    async (data) => {
      try {
        ...
        let artigo = null;
        let artigoFinalizadoAgora =
          false;

        const agora =
          new Date();

        let tempoTotalArtigo =
          0;
  ...
  .then(async () => {
    ...
    http.listen(
      ...
      () => {
        ...
        console.log(
          'Rota PIX antiga: /api/device/activation'
        );

        console.log(
          'Provisionamento: /api/device-provisioning'
        );
      }
    );
  })
